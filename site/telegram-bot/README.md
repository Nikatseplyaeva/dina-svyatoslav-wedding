...
Paste the token and chat ID into [`../js/config.js`](../js/config.js):

```js
window.TELEGRAM_BOT_TOKEN = '123456789:AAExampleTokenExampleTokenExample';
window.TELEGRAM_CHAT_IDS = ['123456789'];
```
...
To have several people each get their own personal-DM copy of every RSVP
(instead of one shared group chat), repeat step 2 for each person — they
must message the bot first so it can find their chat ID — then list all the
IDs:

```js
window.TELEGRAM_CHAT_IDS = ['689587636', '140721868'];
```

The site sends to everyone in the list; if delivery fails for one person it
still succeeds for the others (and logs a console warning for the failed
one).
